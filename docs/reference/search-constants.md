...
| `articleBodyChars` | 100000 | `sources/rss.js` | arbitrary, bounded by a measurement | How much of a feed entry's body is kept at ingest. It read 2000 as an unnamed literal until 2026-08-18 15:39:55 +0200, and `docs/plans/article-body-cap.md` section 1 measures what that cost: of 2509 Guardian articles in this project's own corpus, 2380 are longer than 2000 characters and a median article of 4128 kept 48 percent of itself. The new value is above the longest article ever seen here, 66890, so no real article is cut and the number is a guard against a feed that returns a whole page. Forced by an item actually truncated at this cap, which would mean it has become a decision about content again |
| `explainerDocumentChars` | 2000 | `explainer.js` | arbitrary | How much of the document the explanation call sees. It was an unnamed literal that happened to equal the old ingest cap, which is why it never looked like a decision. Forced by an explanation that misses what the second half of an article says |
| `explainerQueryChars` | 1000 | `explainer.js` | arbitrary | The same for the person's own text. Forced by a truncated intent producing a wrong explanation |
| `explainerMaxTokens` | 256 | `explainer.js` | arbitrary | An API call shape, not a quality number. Found at 2026-08-18 15:52 +0200 when the lint rule was extended to this file, which makes it the fourth language model call in this repository whose shape was configured inline. Forced by an explanation truncated mid sentence |
| `explainerTemperature` | 0.2 | `explainer.js` | arbitrary | Not zero, so the same result can be explained two ways on two runs. Same finding and same moment as the row above, and the same concern phase 2 recorded against `summaryTemperature` and `keywordTemperature` |
...
| `judgeArticleChars` | 2000 | arbitrary, since 2026-08-18 15:39:55 +0200 | It read borrowed, justified as matching `MAX_BODY_LENGTH` in `src/sources/rss.js` so that the judge saw exactly what the system indexed. `docs/plans/article-body-cap.md` raised the ingest cap to `articleBodyChars` and that justification stopped being true, so the row says what it is rather than what it used to lean on. The value is unchanged, because `eval/judgments.json` was graded at it and moving it would require a re-judging pass and a new calibration. Forced by that pass being funded, or by a hand check disagreeing with a judgment because the judge read a fragment |
